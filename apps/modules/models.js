...
module.exports = function initNgModule(ngModule){
    var assert = require('assert'),
        ...
        schemas = require('../../lib/db/models/client.js');
    ngModule.provider('models', function (){
        this.$get = function ($resource, $q) {
            var models = {
                $urlPrefix: settings.serviceUrl.replace(/\/$/, '') + '/models'
            };
                ...
                this[name] = class extends $resource(models.$urlPrefix + '/:name/:id', {name: name.toLowerCase(), id: '@_id'}, actions) {
                    ...
                    clean(){
                        ...
                        angular.forEach(data.schema.paths, function(path, name){
                            ...
                            else if(pathUtil.isRef(path) && data[name] && typeof(data[name])=='object' && !(data[name] instanceof mongoose.Types.ObjectId)){
                                var model = models[path.options.ref];
                                obj[name] = (new model(data[name])).clean();
                            }
                            else {
